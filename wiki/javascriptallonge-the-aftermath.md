---
page_id: javascriptallonge-the-aftermath
page_kind: source
summary: the aftermath from raw/javascriptallonge.pdf.
sources: raw/javascriptallonge.pdf p.269-270
updated: 2026-06-23
source_id: javascriptallonge.pdf
---

## Source record

Chapter on the aftermath of a coding interview scenario in 'JavaScript Allongé'

## Key supported claims

- The Carpenter's solution explores lazy versus eager evaluation and iterator performance (raw/javascriptallonge.pdf p.269-270).
- The Carpenter's code is not meant for production, but illustrates a conversation starter (raw/javascriptallonge.pdf p.269-270).
- The interview leads to a job offer from a stealth startup, FOG (raw/javascriptallonge.pdf p.269-270).

## Technical details

### `technical-atom-c79ad9d6bfff3345` procedure

Citation: (raw/javascriptallonge.pdf p.269-270)

The Carpenter never did hear back from them, but the next day there was an email containing a generous contract from Friends of Ghosts ('FOG'), a codename for a stealth startup doing interesting work, and the Thing interview was forgotten.

### `technical-atom-1305c61697bfdf4d` procedure

Citation: (raw/javascriptallonge.pdf p.269-270)

I thought you'd finally make your escape from New York.'

## Related technical details

### From [[javascriptallonge-summary]]: `technical-atom-c0465a677414b7ed` procedure

Relation: nearby source page; matched terms `code`, `iterator`, `method`

Citation: (raw/javascriptallonge.pdf p.99)

Using a generator instead of writing an iterator object that has a .next() method allows us to write code that can be much simpler for cases like recursive iterations or state patterns.
