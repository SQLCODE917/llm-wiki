...
projection_coverage: topic-javascriptallonge-second@b252e484fcc3cc715e0363a6c7681f69
---
...
- [[javascriptallonge-third]] - shared statements and technical atoms: Third shares source evidence from Picking the Bean: Choice and Truthiness / truthiness and the ternary operator: The fact that either the second or the third (but not both) expressions are evaluated can have important repercussions. Consider this hypothetical example:; Third shares technical record from Picking the Bean: Choice and Truthiness / truthiness and the ternary operator: const status = isAuthorized(currentUser) ? deleteRecord(currentRecord) : 'Forbid\ den'; (1 shared statement(s), 1 shared atom(s))
