...
def main() -> int:
    ...
    parser.add_argument("--normalized-source",
                        help="path to normalized source markdown")
    parser.add_argument(
        "--slug", help="source slug (auto-finds normalized source)")
    ...
    if not source_path.exists():
        print(
            f"FAIL: normalized source not found: {source_path}", file=sys.stderr)
        return 1
            ...
            if cols and len(cols) >= max(evidence_col_idx, locator_col_idx) + 1:
                ...
                if parsed is not None:
                    ...
                    if 1 <= start_line <= len(source_lines):
                        actual_text = get_evidence_excerpt(
                            source_lines, start_line, end_line, max_chars)

                        # Decide whether to replace
                        evidence_clean = evidence.strip('"\'').strip()
                        actual_clean = actual_text.strip()

                        should_replace = force or not texts_match(
                            evidence_clean, actual_clean)
...
def get_evidence_excerpt(source_lines: list[str], start_line: int, end_line: int | None, max_chars: int) -> str:
    ...
    excerpt_lines = [source_lines[i].strip()
