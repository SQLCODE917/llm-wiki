...
def main() -> int:
    parser = argparse.ArgumentParser(description="Cloud-native wiki ingest pipeline")
    parser.add_argument("source", nargs="?", help="source file under raw/inbox/")
    parser.add_argument("--slug", required=True, help="source slug")
    parser.add_argument("--backend", default=os.environ.get("WIKI_MODEL_BACKEND", "bedrock"))
    ...
    if not args.skip_phase0:
        ...
        if args.dry_run:
            ...
            code = run_phase0(state, args.reuse_imported, args.overwrite_normalized)
            if code != 0:
                return code
    ...
    if not state.normalized_source:
        print(f"FAIL: no normalized source found for {state.slug}", file=sys.stderr)
        return 2
    ...
    if not state.source_page.exists():
        print(f"FAIL: source page does not exist: {state.source_page}", file=sys.stderr)
        return 2
    ...
    if not args.skip_phase2:
        ...
        print(f"{'='*60}")
        if args.dry_run:
            print(f"[dry-run] Would synthesize up to {args.max_phase2_pages} pages")
        else:
...
def run_phase0(state: IngestState, reuse_imported: bool, overwrite_normalized: bool) -> int:
    ...
    cmd = ["python3", "tools/wiki_phase0_import.py", str(state.source_path), state.slug]
    if reuse_imported:
...
def run_phase1(state: IngestState) -> int:
    ...
    # Build prompt
    prompt = build_phase1_prompt(state.slug, outline, samples_text, source_lines)
    ...
    source_page_content = postprocess_source_page(source_page_content, state.slug)
...
def run_phase2(state: IngestState) -> int:
    ...
    print(f"Found {len(candidates)} candidates, processing up to {state.max_phase2_pages}")
...
def get_phase2_candidates(source_page: Path) -> list[dict]:
    ...
    candidates.sort(key=lambda c: priority_order.get(c.get('priority', '').lower(), 99))
...
def synthesize_page(backend, state: IngestState, candidate: dict, source_text: str) -> Path | None:
    ...
    relevant_lines = find_relevant_lines(source_text, candidate['title'], candidate.get('evidence', ''))
...
def run_phase3(state: IngestState) -> int:
    ...
    cmd = ["python3", "tools/wiki_log.py", "ingest", f"Cloud ingest of {state.slug}"]
    for d in details:
        cmd.extend(["--detail", d])
    subprocess.run(cmd)
