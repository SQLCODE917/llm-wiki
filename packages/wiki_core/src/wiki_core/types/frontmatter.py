...
# Valid page types
PageType = Literal["source", "entity", "concept",
...
class Frontmatter:
    ...
    source_ranges: list[str] = field(default_factory=lambda: [])
    ...
    def to_yaml(self) -> str:
        ...
        Uses PyYAML for proper escaping and serialization.
        Field order is controlled for readability.
        """
        # Build ordered dict for controlled field order
        data: dict[str, Any] = {}
        ...
        # Source-specific fields (for source pages)
        if self.source_id is not None:
            data["source_id"] = self.source_id
        if self.source_type is not None:
            data["source_type"] = self.source_type
        if self.raw_path is not None:
            data["raw_path"] = self.raw_path
        if self.normalized_path is not None:
            data["normalized_path"] = self.normalized_path
        ...
        if self.source_ranges:
            data["source_ranges"] = self.source_ranges

        # Use PyYAML for proper serialization
