...
- Ordinary ingest is compiler-owned. The harness builds page targets,
  evidence packs, citations, frontmatter, source trails, article lint, staged
...
- Generated compiler pages may include `domain`, `category_path`, `source_id`,
  ...
  justified the page. Treat them like index.md entries: useful for navigation
  and audits, never hand-authored evidence.
...
3. Build a `PagePublicationPlan` from typed evidence and source-map structure:
   procedures from `procedure_step` records, recipes from code examples plus
   nearby argument or definition records, topic pages from accepted
   rule/definition/entity/table/formula records, collections from supported
   high-level sections, and section-reference candidates only when explicitly
   accepted by the publication budget.
...
1. Load persisted compiler artifacts from completed ingests.
2. Plan shared source-backed topics only when at least two sources support the
   same topic key. Each source keeps its own position, citation, evidence-pack
   support, and source-map anchors; do not merge claims into uncited
   generalities.
