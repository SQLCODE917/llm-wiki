...
- `wiki/` — the knowledge base: interlinked markdown pages. Ordinary ingest
  projections are generated by the harness from source artifacts; model-authored
  maintenance/chat writes go through the `write_page` tool.
- `harness/cache/page-plans/<source-run>/ledger/` — generated ingest artifacts:
  the claim ledger, section plan, knowledge-shape catalog, topic index,
  source plan, staged page set, projection lint run, publish run, coverage
  maps, and quality reports that explain how a raw source became wiki pages.
  These artifacts are rebuildable from `raw/` and never count as cited source
  evidence.
...
- Every page has one page kind:
  ...
  - `recipe` — a reusable source-backed pattern with applicability evidence
    and preserved technical atoms, not an ordered execution workflow.
  - `synthesis` — cross-source analysis, comparisons, answers worth keeping.
- Generated projection pages use page families for deterministic navigation:
  `source-manifest`, `section-reference`, `topic-concept`, `broad-topic`,
  `procedure-guide`, `recipe-pattern`, `collection-page`, `entity-profile`,
...
- Generated projection pages may include `domain`, `category_path`,
  `source_id`, and `projection_coverage` frontmatter. These fields are composed
  by the harness from `PageMetadata` and point back to the ledger/coverage
  artifact that justified the page. Treat them like index.md entries: useful for
  navigation and audits, never hand-authored evidence.
...
1. Read the immutable raw source and extract a `PagePlan`. PDF sources also
   build a document model, section structure, and table candidates with
   defense-in-depth recovery: structured rows where possible, raw table text
   when structure is uncertain, and quality findings instead of silent loss.
2. Build an evidence registry, source segments, segment claims, technical
   atoms, and technical-atom contexts from the extracted source material.
...
4. Build the section plan, knowledge-shape catalog, topic index, procedure
   index, recipe candidates, collection candidates, and staged source plan from
   the ledger. Categories, topics, task guides, reusable patterns, and
   collection pages should emerge from source structure, source terminology,
   observed claim/atom clusters, ordered steps, decision points, technical
...
6. Stage projected pages, run deterministic projection lint, and publish only
   accepted pages. The lint run blocks unplanned page kinds/families, missing
   raw-source support, empty bodies, unreasonsed related links, and missing
   technical-atom anchors before any page is returned for writing.
7. Write pages, index entries, generated graph/candidate artifacts, and log
   ...
   maintenance, and curator-directed synthesis.
...
1. For content questions, search the wiki (`search_wiki`), then read the
   relevant pages. In chat, use `inspect_page` before `read_page` for complex
   or large pages; `inspect_page` returns headings, offsets, related pages, and
   source-range hints so reads stay targeted. For task questions ("how do I...",
   setup, creation, workflow, or ordered-use questions), prefer `procedure`
   pages when present. For reusable pattern questions, prefer `recipe` pages
   when present. For questions about the wiki itself or its coverage,
