...
CHAT_TEMPLATE = _BASE + (
    ...
    "first message may include either the wiki index for catalog questions "
    "or initial search hits for content questions. Use those as navigation "
    "hints, then read relevant pages before answering substantively — index "
    "summaries and search snippets alone are too thin for detailed claims. "
    "When you read wiki pages for an answer, include at least one link to a "
    "page you read, such as [[page-id]]; raw source citations alone are not "
    "enough. "
    "In an ongoing conversation, treat a self-contained new topic or noun "
    "phrase as a fresh wiki lookup, not as a reference to the prior answer. "
    "Questions about the wiki itself or its coverage are answered from the "
    "index (read_index re-shows it), never from the schema. Ordinary "
    ...
    "answer, re-verify it against the wiki's current pages. When the user asks "
    "you to execute a procedure and the submit_procedure_execution tool is "
    "available, submit a typed ProcedureExecution before responding. Deliver every "
